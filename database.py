...
def insert_sample_data():
    ...
    try:
        ...
        # 獲取交易ID
        cursor.execute("SELECT tNo FROM TransactionRecord")
        transaction_ids = cursor.fetchall()
        print(f"獲取到的交易ID: {transaction_ids}")
        ...
        try:
            cursor.execute(
                ...
                VALUES 
                ({transaction_ids[0][0]}, 'P001', 500, 1, 'cash', 'product'),
                ({transaction_ids[1][0]}, 'P002', 1000, 2, 'credit card', 'product'),
                ({transaction_ids[2][0]}, 'M001', 1500, 1, 'e-transfer', 'membership')
            """
            )
            print("✓ 訂單插入成功")
            ...
            cursor.execute(
                ...
                SELECT o.* 
                FROM OrderTable o
                LEFT JOIN Product p ON o.gsNo = p.gsNo AND o.orderType = 'product'
                LEFT JOIN MembershipPlan m ON o.gsNo = m.gsNo AND o.orderType = 'membership'
                WHERE (o.orderType = 'product' AND p.gsNo IS NULL)
                   OR (o.orderType = 'membership' AND m.gsNo IS NULL)
            """
            )
            invalid_orders = cursor.fetchall()
            if invalid_orders:
                raise sqlite3.IntegrityError("訂單參照了不存在的商品或會籍")
            ...
            for t_id in transaction_ids:
                cursor.execute(
                    f"SELECT COUNT(*) FROM TransactionRecord WHERE tNo = {t_id[0]}"
                )
                count = cursor.fetchone()[0]
                print(f"TransactionRecord tNo={t_id[0]} 存在數量: {count}")

            cursor.execute("SELECT COUNT(*) FROM Product WHERE gsNo = 'P001'")
            print(f"Product gsNo='P001' 存在數量: {cursor.fetchone()[0]}")

            cursor.execute("SELECT COUNT(*) FROM MembershipPlan WHERE gsNo = 'M001'")
            print(f"MembershipPlan gsNo='M001' 存在數量: {cursor.fetchone()[0]}")
            raise
